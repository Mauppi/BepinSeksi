...
![Version](https://img.shields.io/badge/version-1.4.0-cc0000?style=flat-square)
...
- <strong style="color: #cc0000;">Spectators can vote to leave</strong> — dead players get the vanilla "Tell autopilot ship to leave early" prompt on Gordion, so the last one alive is warned and the round can wrap up.
...
Three things go the other way — they sit behind the same `planetHasTime` check and so never happen at the Company, and the mod supplies them itself: `normalizedTimeOfDay` (vanilla only updates it inside `if (sunAnimator != null)`, and progressing weather is driven by it), the HUD clock (see below), and the midnight departure with its warning (the whole branch lives in `TimeOfDayEvents` behind that flag, so the mod calls the game's own networked `SetShipToLeaveOnMidnightClientRpc`).

The clock took three attempts because alpha was never the problem. The clock shares a HUD cluster with the profit quota — `HUDManager.Clock.canvasGroup` sits on the object named `ProfitQuota` — and that cluster is revealed by an Animator rather than by alpha, in a line gated on `spawnEnemiesAndScrap && planetHasTime`. Gordion has neither, so the animator is never told to show and the cluster stays scaled to nothing. Everything else was already correct: alpha 1, object active, icon set, text reading the right time and ticking up second by second, all of it invisible. The mod now sets that animator itself and keeps checking it, because vanilla's indoor logic still flips the alpha underneath.

The spectator vote is the same story from the other side. None of the vote itself is disabled at the Company — `VoteShipToLeaveEarly`, the server-side count and the warning every player receives never look at the moon. What hides it is one line in `HUDManager.Update`, `if (shipIsLeaving || !currentLevel.planetHasTime)`, which is the only read of that flag anywhere in HUDManager. A transpiler routes exactly that read through a check that answers differently only on Gordion while its day is running, so the rest of the vanilla flow runs untouched: the prompt, the three-second hold, the vote count, "Ship leaving in one hour" and the warning to whoever is still alive. The departure it schedules normally happens in `TimeOfDayEvents`, behind the same flag, so the mod's time driver performs it — and honours a passed vote even when end-of-day departure is switched off, since the players asked for it.
...
- The spectator vote is entirely vanilla: votes go through `SetShipLeaveEarlyServerRpc`, the host counts them against the number of dead players, and every client receives the same warning and deadline. The mod only un-hides the prompt, and only dead players ever see it.
...
| 1. General | `Clock diagnostics` | `false` | Explains in the log why the HUD clock is or is not on screen. |
...
| 3. Time | `Spectators can vote to leave` | `true` | RealTime only. Dead players get the vanilla leave-early vote; a passed vote departs even with the setting above off. |
...
- <strong style="color: #cc0000;">Наблюдатели могут проголосовать за отлёт</strong> — мёртвые игроки получают на Гордионе ванильную подсказку «Tell autopilot ship to leave early», чтобы последний живой получил предупреждение и раунд быстрее закончился.
...
Три вещи, наоборот, приходится доделывать самим — они спрятаны за той же проверкой `planetHasTime` и на Компании не происходят никогда: `normalizedTimeOfDay` (ваниль обновляет его только внутри `if (sunAnimator != null)`, а именно от него зависит прогрессирующая погода), часы в HUD (см. ниже) и полуночный отлёт с предупреждением (вся ветка живёт в `TimeOfDayEvents` за этим флагом, поэтому мод вызывает штатный сетевой `SetShipToLeaveOnMidnightClientRpc` игры).

С часами пришлось повозиться трижды, потому что дело было не в прозрачности. Часы делят блок HUD с квотой прибыли — `HUDManager.Clock.canvasGroup` висит на объекте `ProfitQuota`, — и показывается этот блок Animator'ом, а не альфой, строкой под условием `spawnEnemiesAndScrap && planetHasTime`. У Гордиона нет ни того, ни другого, поэтому аниматору никто не говорит «покажись», и блок остаётся в скрытом состоянии, сжатым в ноль. Всё остальное при этом было правильно: альфа 1, объект активен, иконка на месте, текст показывал верное время и тикал каждую секунду — и всё это невидимо. Теперь мод выставляет этот аниматор сам и продолжает проверять его, потому что ванильная логика «внутри помещения» по-прежнему дёргает альфу.

С голосованием наблюдателей та же история, только с другой стороны. Само голосование на Компании не выключено: `VoteShipToLeaveEarly`, подсчёт голосов на сервере и предупреждение всем игрокам на луну не смотрят. Прячет его одна строка в `HUDManager.Update` — `if (shipIsLeaving || !currentLevel.planetHasTime)`, — и это единственное чтение флага во всём HUDManager. Транспайлер перенаправляет ровно это чтение в проверку, которая отвечает иначе только на Гордионе с идущим днём, а весь остальной ванильный поток работает как есть: подсказка, удержание три секунды, счётчик голосов, «Ship leaving in one hour» и предупреждение тому, кто ещё жив. Сам отлёт ваниль делает в `TimeOfDayEvents` за тем же флагом, поэтому его выполняет драйвер времени мода — и выполняет прошедшее голосование даже при выключенном отлёте в конце дня, потому что об этом попросили игроки.
...
- Голосование наблюдателей полностью ванильное: голоса идут через `SetShipLeaveEarlyServerRpc`, хост считает их против числа мёртвых игроков, и все клиенты получают одинаковое предупреждение и время отлёта. Мод лишь снимает скрытие с подсказки, и видят её только мёртвые игроки.
...
| 1. General | `Clock diagnostics` | `false` | Объясняет в логе, почему часы в HUD видны или нет. |
...
| 3. Time | `Spectators can vote to leave` | `true` | Только RealTime. Мёртвые игроки получают ванильное голосование за отлёт; прошедшее голосование отправляет корабль даже при выключенной настройке выше. |
