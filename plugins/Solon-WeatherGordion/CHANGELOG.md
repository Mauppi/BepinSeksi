...
- **New: spectators can vote for the ship to leave early on Gordion.** Dead players get the vanilla
  "Tell autopilot ship to leave early" prompt, so the last one alive is warned and the round can end.
  None of the vote itself was ever disabled at the Company — `VoteShipToLeaveEarly`, the server-side
  count and the warning to every player never look at the moon. The only thing hiding it is one line
  in `HUDManager.Update`, `if (shipIsLeaving || !currentLevel.planetHasTime)`, which is also the only
  read of that flag anywhere in HUDManager (checked against the game's IL). A transpiler routes that
  single read through a check that answers differently only on Gordion while its day is running; the
  rest of the vanilla flow — prompt, three-second hold, vote count, "Ship leaving in one hour" and the
  warning — runs as shipped.
- The departure a passed vote schedules normally happens in `TimeOfDayEvents`, behind the same flag,
  so the time driver performs it through the same `SetShipToLeaveOnMidnightClientRpc`. A passed vote
  departs even when `Ship leaves at end of day` is off, because that is what the players asked for.
- The 90%-of-day warning no longer repeats when a warning has already gone out; vanilla checks
  `shipLeavingAlertCalled` for exactly this, and the vote's own announcement sets it.
- New `[3. Time] Spectators can vote to leave`, on by default. RealTime only: a vote to leave "in one
  hour" means nothing on a moon whose clock does not move.
- The `Clock tick` heartbeat from the 1.2.2 diagnostics now only logs while `Clock diagnostics` is on;
  it had been writing a line every 30 seconds for everyone.

## 1.3.0

- **The HUD clock is on screen.** Alpha was never the problem, which is why two fixes aimed at it
  changed nothing. The clock shares a HUD cluster with the profit quota — `HUDManager.Clock.canvasGroup`
  sits on the object named `ProfitQuota` — and that cluster is revealed by an Animator, in a line gated
  on `spawnEnemiesAndScrap && planetHasTime`. Gordion has neither, so the animator was never told to
  show and the cluster stayed scaled to nothing. Everything else had been correct all along: the
  diagnostic build reported alpha 1, the object active in the hierarchy, the icon set, and the text
  reading "07:41" and ticking up second by second — all of it invisible. The mod now sets that animator
  and re-checks it each frame, since vanilla's indoor logic still flips the alpha underneath, and
  clears it again on leaving.
- The diagnostics from 1.2.2 stay in, now off by default: `[1. General] Clock diagnostics`. Turn it on
  if the clock ever goes missing again and the log will name the link that broke.

## 1.2.2 — diagnostic build, not for release

The HUD clock has failed to appear twice now, after two fixes that each looked right. Both went
unverified because everything in the drawing path logged through `DebugMode`, which is off by
default. This build changes no behaviour; it only makes the failure visible.

- `Clock chain:` — the full state of the vanilla element on landing and again once the HUD has
  settled: the element and canvas group, every ancestor above it with its active flag, the canvas,
  the rect, the text and icon, and the time values behind them.
- `Clock patches:` — every mod that has patched `HUDManager.SetClockVisible`, `SetClock`,
  `SetClockIcon`, `Update` and `TimeOfDay.SetInsideLightingDimness`, by name, via
  `Harmony.GetPatchInfo`. Names the interference instead of guessing at it.
- `Clock alpha at frame start:` — read before this mod writes anything, which separates "never set"
  from "set and then cleared again between frames".
- `Clock tick #N` — proof the time driver is running at all, with the clock values as it goes.
- Each HUD call is now guarded on its own, so a throw in one no longer skips the alpha write that
  actually shows the clock, and the failure is reported once at warning level naming the step.
- New `[1. General] Clock diagnostics`, on by default in this build only.
