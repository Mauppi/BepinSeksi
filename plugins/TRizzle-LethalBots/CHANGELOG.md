...
## 12.1.4 human-join latency and transport diagnostics hotfix 2026-09-03

- Added an early human-join critical window beginning at Steam lobby/relay connection time. New LLM, TTS, generated-audio decoding, faster-whisper dispatch, ambient dialogue, memory writes, and verbose diagnostic output yield briefly while Steam and Netcode admit the player; queued bot replies resume afterward instead of being discarded.
- Removed the global release-time `UnityEngine.Debug.LogError` interception that synchronously generated and wrote a full managed stack trace for every Unity error.
- Made verbose LethalBots debug logging opt-in for new configurations and suppresses it during the latency-critical join window. Normal join fingerprints, approvals, warnings, and failures remain visible.
- Added stage-specific Facepunch/Steam relay diagnostics on both peers, including connection state, `NetConnectionEnd` code, native relay detail, and elapsed time. Pre-approval client failures now show this real transport stage instead of always claiming a generic mod mismatch.
- All players must install this exact 12.1.4 package; its network compatibility fingerprint intentionally differs from 12.1.3.

## 12.1.3 network-prefab registration hotfix 2026-09-03

- Fixed duplicate `GlobalObjectIdHash` errors for `LethalBotManager`, `SaveManager`, `TerminalManager`, and `GroupManager` introduced by the 12.1.2 early network-prefab repair.
- Restored a single registration owner: LethalLib performs normal registration during `GameNetworkManager.Start`, while LethalBots now only repairs a genuinely missing entry immediately before network startup.
- Added hash-aware duplicate and collision checks so an equivalent already-registered prefab is skipped without emitting NGO errors, while a real cross-prefab hash collision remains visible and blocks a misleading repair.
- All players must install this exact 12.1.3 package; its network-prefab compatibility hashes intentionally differ from 12.1.2.

## 12.1.2 multiplayer join reliability update 2026-09-03

- Fixed silent pre-approval disconnects caused by inconsistent runtime network-prefab registration and hashes.
- Restored the NetworkManager and late-join scene synchronization guards lost during integration.
- Human joins now pause bot quota work, reserve distinct player slots, and take priority over bot spawning; an in-flight bot spawn is allowed to settle briefly instead of rejecting the player.
- Network configuration failures now reach the joining player's menu with an actionable reason and produce a comparable host/client handshake fingerprint in both logs.
- All players must install this exact 12.1.2 package because LethalBots contains required network behaviours and prefabs.

## 12.1.1 local multiplayer voice and soundboard update 2026-09-03

- Made authenticated direct player addresses response-required through the entire generated-dialogue pipeline. They now supersede older incidental requests for the same bot, outrank lower-value human interjections during preparation, remain queued behind already-audible speech for up to 60 seconds, explicitly forbid model-selected silence, and use a safe spoken acknowledgement if generation is empty, malformed, or filtered.
- Removed the legacy 110-character/18-word hard slice that could stop generated dialogue mid-sentence. Ordinary replies now target complete one-to-three-sentence thoughts, default to a configurable 48-word ceiling (12-72), allow a proportionate character budget, and only shorten exceptional overlong output at a late sentence boundary or whole word; short danger reflexes remain separately bounded.
- Prioritized human voice/chat replies over scripted, bot-to-bot, event, and ambient generation. Directly addressed requests no longer disappear behind broad reactive cooldowns, stale lower-priority preparation is cancelled, and one reply can wait behind an already-audible sentence without letting autonomous work retake the LLM/TTS lane; danger reactions remain highest priority.
- Preserved authenticated non-host microphone forwarding while retaining the validated walkie state and speaking position captured with the utterance instead of replacing them after upload/transcription delay.
- Made the bounded faster-whisper transcription queue fair between player IDs, preventing one noisy or highly active speaker from evicting every other player's pending segment.
- Prioritized active bot names and round-robin aliases in the larger transcription hotword budget, added unique edge-position fuzzy address hints, and kept ambiguous names from selecting a bot.
- Added a host-authoritative `play_soundboard` LLM tool for random or explicitly named `.wav`, `.ogg`, and `.mp3` clips. It is now explicitly presented as a live expressive capability and strongly encouraged for frequent context-appropriate proactive use during casual human replies, ambient silence, spawns, and ordinary events—not only literal sound requests. Its live filename enum is reshuffled for every eligible request, path input is catalog-only, clips are size/cooldown bounded, and bytes use the existing paced bot-voice broadcast so every client hears the same positional source without a local soundboard copy. Clear gameplay requests remain protected from clip substitution, and soundboard use no longer consumes the longer autonomous gameplay-tool cooldown.

## 12.1.1 local danger-navigation update 2026-08-23

- Removed every visible-body teleport fallback from environmental and danger-blocked navigation. Turrets, traps, water/quicksand, enemy exposure, and danger-rated objective routes now remain physical movement problems; normal entrance use and non-danger geometry recovery stay separate.
- Added immediate one-leg cover/retreat decisions plus rear and side NavMesh candidates, so bots can move to safety and reassess even when no candidate already has a complete safe second leg to the objective.
- Reduced hazard reaction/retry latency, increased bounded candidate work per frame, and made bots replan quickly rather than waiting several seconds in danger.
- Improved turret escape evaluation so a bot already in a firing lane can move laterally toward real line-of-sight cover without treating every non-radial escape step as an invalid route.
- Limited off-NavMesh danger recovery to realigning the invisible owner-side navigation agent with the unchanged visible body; no teleport RPC is sent.
- Removed the shared terminal-navigation body warp used after ordinary route failure, including `ReturnToShip`. Bots now physically walk to a reachable recovery node, retry their goal, or remain in place, and follow catch-up cannot warp them into the ship. Real entrances, ladders/elevators, purchased teleporters, spawn, and departure transitions are unchanged.
- Replaced outdoor-enemy fear radii as last-resort path walls with bounded necessary crossings. Bots still prefer complete safe detours, but healthy bots can sprint through a passive threat's visible radius when that is the only usable route.
- Prevented repeated LOS panic from cancelling the crossing the bot just chose. The owner-local commitment follows visible-body progress and is revoked on stalls, low health, close/imminent attacks, or state/goal changes; active pursuit raises the required clearance instead of recreating a distant radial wall, while defended hives and live slime remain hard contact hazards.

## 12.1.1 local crew participation update 2026-08-23

- Added synchronized options requiring every available bot to take part in one field/scrap run per normal expedition and to help with Company-moon selling, including bots that would otherwise remain on mission control.
- Fixed start-of-day participation being marked complete when a bot merely entered `SearchingForScrap`; completion now requires sustained physical movement beyond the ship perimeter or entry into the facility, and failed deployments remain eligible for immediate retry.
- Made automatic mission-control claims server-atomic and deferred ship duty for bots that still owe a run, preventing multiple clients from racing for the same passive role.
- Kept intentionally issued self-defense scrap weapons, such as Stop signs, classified as field equipment so an armed bot no longer loops straight back to the ship believing its weapon is collected cargo.
- Added per-bot, per-round participation tracking and staggered deployment. Natural work satisfies the policy, while explicit directed tasks, combat, rescue, purchases, Eclipse, active emergencies, and late-day safety remain authoritative overrides; static hard-moon risk now favors arming the bot instead of excusing its run.
- Preserved explicit follow orders from chat, voice, direct controls, and the grounded LLM `follow_player` tool during a bot's first required run. The request bit now follows bot ownership to the controlling client, follow/lost-player states are no longer overwritten by the participation watchdog, and only sustained physical deployment with the requested leader completes the run.
- Made concurrent Company selling reserve another bot's current target and fixed **Sell all scrap on ship** selection so reaching quota no longer stops that explicit sell-everything mode.
- Extended ship-door control to any living bot physically inside the ship and moved arbitration to the host so multiple idle or mission-control bots cannot fight over it. Bots now close for classified immediate threats near or inside the ship (or after return time when everyone is safely inside), reopen for living surface crew within 12 metres, hold the door for crossing, respect power/overheat/animation constraints, and yield briefly to manual player input instead of disabling the feature for the whole round.

## 12.1.1 local AI agency update 2026-08-22

- Added deterministic round-risk assessment from moon rank, enemy budgets, facility scale, weather, quota cycle, and quota pressure; generated dialogue now sees this plus defense readiness, round phase, mission-control ownership, and richer named nearby-crew context.
- Added adaptive self-defense loadouts. Unarmed bots on hard or late-run expeditions take one usable existing ship weapon before deploying, avoid storm-conductive gear and already-claimed items, and can be explicitly told to arm themselves through a grounded generated tool.
- Hardened defense-readiness scans against partially initialized inventories and destroyed Unity item references that can remain in the periodically refreshed global item registry.
- Expanded rare malicious autonomy into live, revalidated opportunities: weapon hoarding, nearby reachable player betrayal, or a strictly late crew-abandoning departure. Random global victims and early departures are no longer valid.
- Fixed mineshaft-elevator deadlocks by retaining each bot's requested trip through state changes, allowing stationary cages to resume AI despite their physics parent, using a current-floor NavMesh exit fallback, restoring stuck recovery after the lift stops, and replacing the scene-resetting shared button cooldown clock.
- Fixed multilingual Chatterbox replies by carrying each accepted faster-whisper language through the host-authoritative transcript RPC and exact queued dialogue request, rejecting weak/rejected-clip guesses, prompting the model to keep that language, and selecting the multilingual Chatterbox model for non-English lines. Per-player languages can no longer race through one global last-detected value.

## 12.1.1 local AI reliability update 2026-08-15

- Prevented post-ejection/landing bot duplication with coalesced spawn passes, in-flight body-slot reservations, despawn cleanup, and retained-identity reconciliation.
- Added bounded alternative-route search using lateral, fan-shaped, and map AI-node waypoints; both route legs must be complete and candidates are ranked by danger, length, progress, and clearance.
- Added owner-side physical body-progress tracking so a detached NavMeshAgent can no longer hide an obstacle stall; failed waypoints are avoided before synchronized recovery is used.
- Suspended NavMesh recovery during intentional off-mesh movement such as ladders, ship landing/take-off, Tulip Snake carries, vehicles, elevators, and moving physics parents; resumption now reattaches only the local AI agent without teleporting the bot body.
- Moved bee-route avoidance to the hive's actual defense radius instead of the moving swarm position, while preserving intentional hive collection.
- Made hazard escalation deterministic after repeated safe-route failures. Bots may sprint through the lowest-risk survivable fallback; entry into lethal hazards remains vetoed, while bots already caught in danger may use only outward, survival-checked escape routes.
- Added host-authoritative, tool-free LLM/TTS danger reactions for chases, damage, sinking, landmines, and failed escapes, with severity escalation, crew coalescing, and Responsiveness cooldowns.
- Fixed llama.cpp reasoning-only timeouts by sending an explicit bounded thinking budget, adding that budget to the visible completion allowance, and preventing hidden `<think>` text from being parsed as dialogue or tool output.
- Fixed player self-defense for shovel, knife, shotgun, and compatible melee hits by preserving the verified attacker through actual damage, routing retaliation to the bot's current network owner, and preventing follow/group logic from approaching that aggressor again before hostility expires.
- Made serious player-threat startles preempt stale bot dialogue and generated audio, prioritize playback on every client, allow a bounded two-voice urgent overlap, cancel on death/despawn, and retry quickly when the host rejects a busy reaction without consuming the full cooldown.
- Added randomized, context-aware self-talk for continuously isolated bots, with local-silence timing, Talkativeness-weighted attempts, no gameplay tools or global chat mirroring, and an isolation recheck before playback.
- Added partner-targeted, context-aware bot conversations grounded in current activity, location, health, and held items, with one listener and a hard two-to-three-turn limit.
- Made ambient silence and conversation cooldown settings hard minimums measured from real audible playback.
- Stopped mirrored bot chat and bot-to-bot speech from recursively generating more replies.
- Enforced Talkativeness, Responsiveness, generated-reply, ambient, scripted-speech, dead-bot, and cooldown settings at queue and playback time.
- Bounded/coalesced generated dialogue and audio queues, dropped stale ambient work, and stopped failed TTS from creating phantom speech events.
- Added host-authoritative faster-whisper selection, duplicate-source suppression, transcript noise filtering, sender authentication, and bounded audio reassembly.
- Filtered low-information interjections, repeated laughter/growls, and cut-off vocalizations before they can schedule dialogue.
- Carried voice-state categories through generated-audio RPCs so NoTalking and NoResponses mute the intended lines.
- Hardened LLM tools with explicit direct-command grammar, per-action intent checks, negation and reported-speech handling, grounded targets, single-shot execution, and synchronized owner/state revalidation.
- Expanded grounded generated tools for practical movement, grouping, loadouts, scrap work, healing, charging, inventory use, ship systems, monitoring, rescue, loot transfer, terminal purchases, and bounded direct combat/retreat.
- Added an independently configurable ambient-autonomy lane with per-bot cooldowns, one world-changing action per response, strict round-state checks, and no autonomous player attacks.
- Added utility-weighted supply and practical ship-upgrade buying, strongly de-prioritized unnecessary or duplicate Weed killer, preserved the configured restock reserve, and allowed only an authenticated direct host/captain order to spend below that reserve.
- Serialized item and ship-upgrade purchases through one bounded terminal queue, yielded to human terminal users, revalidated every transaction, and restored temporary mission-control assignments and network ownership after each batch.
- Added optional personality-gated minor sabotage limited to one safe, recoverable action per round: one cheap non-Weed-killer order, one safe horn ring, or terminal abandonment. It shares the autonomous economy cooldown and refuses emergencies, threats, departure, and reserved-credit spending.
- Bound direct-command identity to immutable client IDs, rejected quoted consequential commands and ambiguous player/store targets, and tied tool arguments to the single target or purchase object in the imperative clause.
- Made `shouldSpeak: false` strictly silent and reduced sensitive prompt, transcript, memory, tool-argument, and generated-line logging.
- Added per-save persistent dialogue memory lifecycle, reset/delete hooks, and a zero-retention mode.
- Added safe recovery for corrupt mod saves, in-memory migration of legacy stock requirements, null-safe stock RPC serialization, and identity-aware quota spawning.
- Preserved the Qwen3/Chatterbox/ElevenLabs/SAPI TTS, faster-whisper, personality, memory, directed-task, combat, and autonomous AI extensions on the 12.1.1 upstream base.
